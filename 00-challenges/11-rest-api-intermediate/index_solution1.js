/*
  Technical Interview — REST API: Cart Aggregation

  Using the live dummyjson.com API, complete the following steps:

  1. POST to https://dummyjson.com/auth/login with username "emilys" and
     password "emilyspass" to obtain a short-lived access token.

  2. Use that access token to GET https://dummyjson.com/carts
     (pass the token as a Bearer token in the Authorization header).

  3. Aggregate the product data from all carts.

  4. Identify the top 3 products (by id) for each of the following metrics:
       a. uniqueCartCount   — number of carts containing the product
       b. sumQuantity       — total units ordered across all carts
       c. sumExtendedTotal  — total dollar value ordered across all carts

  Return an object with the three top-3 lists (each a 3-element array of product
  id strings, ranked highest first):

      {
        uniqueCartCount:  ["<id>", "<id>", "<id>"],
        sumQuantity:      ["<id>", "<id>", "<id>"],
        sumExtendedTotal: ["<id>", "<id>", "<id>"],
      }

  This is what "Run Tests" checks. (In the real exercise you would then POST this
  shape to https://dummyjson.com/carts/add as a Bearer-authenticated submission.)

  You may use any publicly available npm package (axios and lodash are installed).
  Run your solution directly with:  node index.js
*/
const axios = require("axios");

async function getTopProducts() {
  // hit the endpoint with a POST request and get an access token
  const response = await axios.post("https://dummyjson.com/auth/login", {
    username: "emilys",
    password: "emilyspass",
  });
  // get the access token from the response object
  const token = response.data.accessToken;
  // use access token to get cart data
  const cartsResponse = await axios.get("https://dummyjson.com/carts", {
    headers: {
      Authorization: `Bearer ${token}`,
    },
  });
  // extract carts objects
  const carts = cartsResponse.data.carts;
  // declare object to hold stats
  const stats = {};
  // double loop to touch each cart, but also, inside each cart, touch each product object
  for (const cart of carts) {
    for (const product of cart.products) {
      if (!stats[product.id]) {
        stats[product.id] = {
          uniqueCartCount: 0,
          sumQuantity: 0,
          sumExtendedTotal: 0,
        };
      }
      // accumulate the time each id appears in each cart
      stats[product.id].uniqueCartCount += 1;
      // add up product quantities
      stats[product.id].sumQuantity += product.quantity;
      // add up the total value in $$
      stats[product.id].sumExtendedTotal += product.total;
    }
  }
  // turn big object of objects into elements of an array for sorting:
  const statsAsArray = Object.entries(stats);
  // sort by top uniqueCartCounts
  statsAsArray.sort((a, b) => b[1].uniqueCartCount - a[1].uniqueCartCount);
  // get the top 3 values
  const topUniqueCartCount = statsAsArray.slice(0, 3);
  // sort by top sumQuantities
  statsAsArray.sort(
    (a, b) =>
      b[1].sumQuantity - a[1].sumQuantity || Number(a[0]) - Number(b[0]),
  );
  // get the top 3 values
  const topSumQuantity = statsAsArray.slice(0, 3);
  // sort by top sumExtendedTotals
  statsAsArray.sort((a, b) => b[1].sumExtendedTotal - a[1].sumExtendedTotal);
  // get top 3 values
  const topSumExtendedTotals = statsAsArray.slice(0, 3);
  return {
    // return values directly, using the map method to only return the ids
    uniqueCartCount: topUniqueCartCount.map((a) => a[0]),
    sumQuantity: topSumQuantity.map((a) => a[0]),
    sumExtendedTotal: topSumExtendedTotals.map((a) => a[0]),
  };
}

module.exports = { getTopProducts };

// Run `node index.js` to execute your solution and print the result.
if (require.main === module) {
  getTopProducts()
    .then((result) => console.log(result))
    .catch((err) => console.error("Error:", err.message));
}
