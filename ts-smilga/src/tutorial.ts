...
In TypeScript, a Tuple is a special type that allows you to create an array where 
...
By default, tuples in TypeScript are not read-only. This means you can modify the 
values of the elements in the tuple. However, TypeScript does provide a way to make 
tuples read-only using the readonly keyword.
*/
let person: [string, number] = ["jimmy", 25];
console.log(person[0]); // Outputs: jimmy
console.log(person[1]); // Outputs: 25

let jimmy: [string, number?] = ["jimmy"];
...
console.log(randomPerson[0]); // Outputs: jimmy
